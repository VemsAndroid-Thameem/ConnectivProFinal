...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('reimbursement.index') }}">{{ __('Reimbursement') }}</a></li>
    <li class="breadcrumb-item active" aria-current="page"></li>{{ __('Reimbursement Detail') }}
@endsection
...
@section('action-button')
    <div class="col-md-12 text-end d-flex ">
        ...
        @if ($chatgpt == 'on')
            <div class="text-end pt-2">
                <a href="#" class="btn btn-sm btn-primary" data-size="medium" data-ajax-popup-over="true"
                    data-url="{{ route('generate', ['reimbursements']) }}" data-bs-toggle="tooltip" data-bs-placement="top"
        ...
        @if (\Auth::user()->type == 'company' || \Auth::user()->type == 'hr' || \Auth::user()->type == 'Accounts' || \Auth::user()->type == 'manager' )
            <a href="{{ route('send.mail.reimbursement', $reimbursement->id) }}" class="btn btn-sm btn-primary btn-icon m-2"
            ...
            <!-- @can('Create Reimbursement')
                {{-- @if (\Auth::user()->type == 'company' && $reimbursement->status == 'accept') --}}

                <a href="#" data-size="lg" data-url="{{ route('reimbursements.copy', $reimbursement->id) }}"data-ajax-popup="true"
        ...
        <a href="{{ route('reimbursement.download.pdf', \Crypt::encrypt($reimbursement->id)) }}"
        ...
        {{-- @if (\Auth::user()->type == 'company' || \Auth::user()->type == 'employee') --}}
        <a href="{{ route('get.reimbursement', $reimbursement->id) }}" target="_blank" class="btn btn-sm btn-primary btn-iconn m-2"
            title="{{ __('Preview') }}" data-bs-toggle="tooltip" data-bs-placement="top">
            <i class="ti ti-eye"></i>
        </a>
        {{-- @endif --}}

        {{-- @if ((\Auth::user()->type == 'company' && $reimbursement->company_signature == '') || (\Auth::user()->type == 'employee' && $reimbursement->employee_signature == ''))
            @if ($reimbursement->status == 'accept')
                <a href="#" class="btn btn-sm btn-primary btn-icon m-2"
                    data-url="{{ route('signature', $reimbursement->id) }}" data-ajax-popup="true"
        ...
        @php
            $status = App\Models\Reimbursement::status();
        @endphp

        @if (\Auth::user()->type == 'company' && $reimbursement->status=="review" && $reimbursement->updated_by=='Accounts'
        || (\Auth::user()->type == 'hr' && $reimbursement->status=="review" && $reimbursement->updated_by=='Accounts') 
        || \Auth::user()->type=='Accounts' && $reimbursement->status == 'review' && $reimbursement->updated_by=='manager'
        || \Auth::user()->type=='manager' && $reimbursement->status=="pending")
            <ul class="list-unstyled mb-0 m-1">
                <li class="dropdown dash-h-item drp-language">
                    <a class="dash-head-link dropdown-toggle arrow-none me-0" data-bs-toggle="dropdown" href="#"
                        role="button" aria-haspopup="false" aria-expanded="false">
                        <span class="drp-text hide-mob text-primary">{{ ucfirst($reimbursement->status) }}
                            <i class="ti ti-chevron-down drp-arrow nocolor hide-mob"></i>
                    </a>
                    <div class="dropdown-menu dash-h-dropdown">
                        <!-- two way approval -->
                        @foreach ($status as $k => $status)
                            @if((\Auth::user()->type == 'Accounts' || \Auth::user()->type=='manager') && ucfirst($status) == 'Accept')
                                @continue
                            @elseif(\Auth::user()->type == 'hr' && ucfirst($status) == 'Review')
                                @continue
                            @endif
                            
                            <a class="dropdown-item status" data-id="{{ $k }}"
                                data-url="{{ route('reimbursement.status', $reimbursement->id) }}"
                        ...
                        <div id="general">
                                        ...
                                        <div class="col-lg-4 col-6">
                                            <div class="card">
                                                <div class="card-body" style="min-height: 205px;">
                                                    ...
                                                    <h3 class="mb-0">{{ count($reimbursement->files) }}</h3>
                                                    <h3 class="mb-0"></h3>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-lg-4 col-6">
                                            <div class="card">
                                                <div class="card-body" style="min-height: 205px;">
                                                    ...
                                                    <h3 class="mb-0">{{ count($reimbursement->comment) }}</h3>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-lg-4 col-6">
                                            <div class="card">
                                                <div class="card-body" style="min-height: 205px;">
                                                    ...
                                                    <h3 class="mb-0">{{ count($reimbursement->note) }}</h3>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="card-body pt-0 ">
                                            <address class="mb-0 text-sm">
                                                <div class="row mt-3 align-items-center">
                                                    <h6>{{ __('Reimbursement Detail') }}</h6>
                                                    <div class="col-sm-4 h6 text-sm">{{ __('Employee Name') }}</div>
                                                    <div class="col-sm-8 text-sm"> {{ $reimbursement->employee->name }}</div>

                                                    <div class="col-sm-4 h6 text-sm">{{ __('Subject') }}</div>
                                                    <div class="col-sm-8 text-sm"> {{ $reimbursement->subject }}</div>

                                                    <div class="col-sm-4 h6 text-sm">{{ __(' Type') }}</div>
                                                    <div class="col-sm-8 text-sm">{{ $reimbursement->reimbursement_type->name }}
                                                    </div>

                                                    <div class="col-sm-4 h6 text-sm">{{ __('Value') }}</div>
                                                    <div class="col-sm-8 text-sm">
                                                        {{ Auth::user()->priceFormat($reimbursement->value) }}</div>

                                                    <div class="col-sm-4 h6 text-sm">{{ __('Start Date') }}</div>
                                                    <div class="col-sm-8 text-sm">
                                                        {{ Auth::user()->dateFormat($reimbursement->start_date) }}</div>

                                                    <div class="col-sm-4 h6 text-sm">{{ __('End Date') }}</div>
                                                    <div class="col-sm-8 text-sm">
                                                        {{ Auth::user()->dateFormat($reimbursement->end_date) }}</div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body p-3">
                                    {{ Form::open(['route' => ['reimbursements.description.store', $reimbursement->id]]) }}
                                    <div class="col-md-12">
                                        <div class="form-group mt-3">
                                            <textarea class="tox-target pc-tinymce-2" name="reimbursement_description" id="reimbursement_description" rows="3">{!! $reimbursement->reimbursement_description !!}</textarea>
                                        </div>
                                    </div>
                                    @can('Create Reimbursement')
                                ...
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            ...
                                            @elseif(\Auth::user()->type == 'employee' && $reimbursement->status == 'accept' || $reimbursement->status == 'pending' || $reimbursement->status == 'review')
                                                <div class=" ">
                                                    <div class="col-md-12 dropzone browse-file" id="my-dropzone"></div>
                                                </div>
                                            @endif
                                                        ...
                                                        <div class="row align-items-center">
                                                            <div class="col">
                                                                ...
                                                                <p class="card-text small text-muted">
                                                                    {{ number_format(\File::size(storage_path('reimbursement_attechment/' . $file->files)) / 1048576, 2) . ' ' . __('MB') }}
                                                                </p>
                                                            </div>
                                                            @php
                                                                $attachments = \App\Models\Utility::get_file('reimbursement_attechment');
                                                            @endphp
                                                                ...
                                                                @can('Delete Attachment')
                                                                    @if (\Auth::user()->id == $file->user_id || \Auth::user()->type == 'company' || \Auth::user()->type == 'hr')
                                                                        <div class="action-btn bg-danger ms-2">
                                                                            ...
                                                                            <form action=""></form>
                                                                            {!! Form::open(['method' => 'GET', 'route' => ['reimbursements.file.delete', [$reimbursement->id, $file->id]]]) !!}
                                                                            <a href="#!"
                                            ...
                                            <div class="card-footer">
                                                    ...
                                                    @if (\Auth::user()->type == 'company' || \Auth::user()->type == 'hr')
                                                        <div class="form-group mb-0 form-send w-100">
                                                            <input type="hidden" id="commenturl"
                                                                value="{{ route('comment.store', $reimbursement->id) }}">
                                                ...
                                                <div class="">
                                                    <div class="list-group list-group-flush mb-0" id="comments">
                                                        @foreach ($reimbursement->comment as $comment)
                                                            @php
                                    ...
                                    <div id="notes">
                                        <div class="card">
                                            ...
                                            <div class="card-body">
                                                ...
                                                @if (\Auth::user()->type == 'company' || \Auth::user()->type == 'hr')
                                                    <form action=""></form>
                                                    {{ Form::open(['route' => ['reimbursements.note.store', $reimbursement->id]]) }}
                                                ...
                                                <div class="">
                                                    <div class="list-group list-group-flush mb-0" id="comments">
                                                        @foreach ($reimbursement->note as $note)
                                                            @php
                                                                ...
                                                                <div class="row align-items-center">
                                                                    ...
                                                                    @can('Delete Note')
                                                                        @if (\Auth::user()->id == $note->user_id || \Auth::user()->type == 'company' || \Auth::user()->type == 'hr')
                                                                            <div class="p-0 w-auto action-btn bg-danger">
                                                                                {!! Form::open(['method' => 'GET', 'route' => ['reimbursements.note.destroy', $note->id]]) !!}
                                                                                <a href="#!"
                                                                                    class=" btn btn-sm d-inline-flex align-items-center bs-pass-para "
                                                                                    data-bs-toggle="tooltip"
                                                                                    data-bs-placement="top"
...
@push('script-page')
    ...
    <script>
        ...
        myDropzone = new Dropzone("#my-dropzone", {
            ...
            url: "{{ route('reimbursements.file.upload', [$reimbursement->id]) }}",
        ...
        myDropzone.on("sending", function(file, xhr, formData) {
            ...
            formData.append("reimbursement_id", {{ $reimbursement->id }});
