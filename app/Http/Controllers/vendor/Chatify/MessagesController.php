...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->getClientOriginalExtension()), $allowed)) {
                    ...
                    Storage::disk('local')->put('attachments/' . $attachment, file_get_contents($file));
        ...
        if (!$error->status) {
            ...
            //send to TEAMS
            $senderName = User::where('id', '=', $request['id'])->get()->value('name');
            \Log::info($senderName);
            $MSmessage = 'Hi '.$senderName.'!, '.Auth::user()->name.' has sent you a message.';
