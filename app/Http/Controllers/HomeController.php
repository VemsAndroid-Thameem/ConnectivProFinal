    ...
    public function index()
    {
        if (Auth::check()) {
            ...
            else
            {
                ...
                // Expense Summary
                $curr_month  = Expense::where('created_by', '=', \Auth::user()->creatorId())->whereMonth('date', '=', date('m'))->get();
                $cnt_expense['this_month']  = \App\Models\Expense::getExpenseSummary($curr_month);

                // Income Summary
                $curr_month  = Deposit::where('created_by', '=', \Auth::user()->creatorId())->whereMonth('date', '=', date('m'))->get();
                $cnt_income['this_month']  = \App\Models\Deposit::getIncomeSummary($curr_month);


                return view('dashboard.dashboard', compact('cnt_expense', 'cnt_income', 'arrEvents', 'announcements', 'employees', 'activeJob', 'inActiveJOb', 'meetings', 'countEmployee', 'countUser', 'countTicket', 'countOpenTicket', 'countCloseTicket', 'notClockIns', 'countEmployee', 'accountBalance', 'totalPayee', 'totalPayer'));
            }
        }
        else
        {
