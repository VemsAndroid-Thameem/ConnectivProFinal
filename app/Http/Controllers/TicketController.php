...
class TicketController extends Controller
{
    public function index()
    {
        ...
        if (\Auth::user()->can('Manage Ticket')) {
            ...
            return view('ticket.index', compact('tickets', 'countTicket', 'countOpenTicket', 'countCloseTicket', 'countonholdTicket', 'ticket_arr', 'managerDepartmentId', 'managerBranchId'));
        } else {
    ...
    public function create()
    {
        ...
        //     if (\Auth::user()->type != 'employee') {
            ...
            if(\Auth::user()->type=='employee'){
                $employees = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'employee')->where('id','=', \Auth::user()->id)->get()->pluck('name', 'id');
            }elseif(\Auth::user()->type == 'manager'){
                $managerDepartmentId = User::where('id', '=', \Auth::user()->id)->get()->value('department_id');
                $managerBranchId = User::where('id', '=', \Auth::user()->id)->get()->value('branch_id');
                $employees = Employee::where('created_by', '=', \Auth::user()->creatorId())->where('branch_id', '=', $managerBranchId)->where('department_id', '=', $managerDepartmentId)->get()->pluck('name', 'user_id');

                // $employees1 = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'employee')->get()->pluck('name', 'id');
                // \Log::info('employees: '.$employees.' user employees: '.$employees1);
            }else{
                $employees = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'employee')->get()->pluck('name', 'id');
            }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Ticket')) {
            ...
            if ($setings['new_ticket'] == 1) {
                ...
                // Mail for Employee (new_ticket template)
                $employeeArr = [
                    'ticket_title' => $ticket->title,
                    'ticket_name' => $employee->name,
                    'ticket_code' => $rand,
                ...
                $respEmployee = Utility::sendEmailTemplate('new_ticket', [$employee->email], $employeeArr);


                // Retrieve selected user IDs based on roles (excluding 'employee')
                $selectedUserIds = DB::table('users')
                ...
                // Mail for selected users (ticket_notification template)
                $selectedUsersArr = [
                    'ticket_title' => $ticket->title,
                    'ticket_name' => $employee->name, // Or change this name if needed
                    'ticket_code' => $rand,
                    'ticket_description' => $request->description,
                ];

                // Remove the employee's email from the list to avoid duplicate emails
                $filteredEmailRecipients = array_diff($selectedUserIds, [$employee->email]);

                $respSelectedUsers = Utility::sendEmailTemplate('ticket_notification', $filteredEmailRecipients, $selectedUsersArr);
                return redirect()->route('ticket.index')->with('success', __('Ticket successfully created.') . ((!empty($respEmployee) && $respEmployee['is_success'] == false && !empty($respEmployee['error'])) ? '<br> <span class="text-danger">' . $respEmployee['error'] . '</span>' : ''));
    ...
    public function edit($ticket)
    {
        ...
        if (\Auth::user()->can('Edit Ticket')) {
            $employees = User::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 'employee')->get()->pluck('name', 'id');
            $emp= $ticket->employee_id;
            $empName= Employee::where('user_id','=',$emp)->get()->value('name');

            return view('ticket.edit', compact('ticket', 'employees', 'empName'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
