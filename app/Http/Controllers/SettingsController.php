...
class SettingsController extends Controller
{
    ...
    public function saveTeamsSettings(Request $request)
    {
        if (\Auth::user()->type == 'company') {
            $request->validate(
                [
                    'teams_channel_webhook_url' => 'required|string|max:255',
                ]
            );

            $arrEnv = [
                'TEAMS_CHANNEL_WEBHOOK_URL' => $request->teams_channel_webhook_url,
            ];
            Utility::setEnvironmentValue($arrEnv);
