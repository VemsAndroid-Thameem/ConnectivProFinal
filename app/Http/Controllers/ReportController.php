...
class ReportController extends Controller
{
    ...
    public function leave(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            ...
            $leaves        = [];
            $totalApproved = $totalReject = $totalPending = $totalReviewed = 0;
            foreach ($employees as $employee) {
                ...
                $reviewed  = Leave::where('employee_id', $employee->id)->where('status', 'Reviewed');
                ...
                if ($request->type == 'monthly' && !empty($request->month)) {
                    ...
                    $reviewed->whereMonth('applied_on', $month)->whereYear('applied_on', $year);
                ...
                if ($request->type == 'yearly' && !empty($request->year)) {
                    ...
                    $reviewed->whereYear('applied_on', $request->year);
                ...
                $employeeLeave['reviewed'] = $reviewed;
            ...
            $filter['totalReviewed'] = $totalReviewed;
    ...
    public function monthlyAttendance(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            ...
            $employeesAttendance = [];
            $totalPresent        = $totalLeave = $totalEarlyLeave = $totalRemote = 0; //Remote initialize variable
                ...
                foreach ($dates as $date) {
                    ...
                    if ($dateFormat <= date('Y-m-d')) {
                        ...
                        if (!empty($employeeAttendance) && $employeeAttendance->status == 'Present') {
                            if($employeeAttendance->isRemote == 1){ //remote login include this if block
                                $attendanceStatus[$date] = 'R';
                                $totalRemote +=1;
                            }else{
            ...
            $data['totalRemote']     = $totalRemote; //remote.. add the data
