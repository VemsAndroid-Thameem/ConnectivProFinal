...
class PromotionController extends Controller
{
    public function index()
    {
        if(\Auth::user()->can('Manage Promotion'))
        {
            ...
            return view('promotion.index', compact('promotions', 'managerDepartmentId', 'managerBranchId'));
        }
        else
        {
    ...
    public function create()
    {
        if(\Auth::user()->can('Create Promotion'))
        {
            if(\Auth::user()->type=='manager'){
                ...
                $designations = Designation::where('created_by', Auth::user()->creatorId())->where('department_id', '=', $managerDepartmentId)->get()->pluck('name', 'id');
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('Create Promotion'))
        {
            ...
            $employeeDB = Employee::where('employee_id', $request->employee_id )->first();
                $employeeDB->designation_id = $request->designation_id;
                $employeeDB->save();
    ...
    public function edit(Promotion $promotion)
    {
        ...
        if(\Auth::user()->type=='manager'){
            $managerDepartmentId = User::where('id', '=', \Auth::user()->id)->get()->value('department_id');
            $managerBranchId = User::where('id', '=', \Auth::user()->id)->get()->value('branch_id');
            $employees    = Employee::where('id', $promotion->employee_id)->get()->pluck('name', 'id');
            $designations = Designation::where('created_by', Auth::user()->creatorId())->where('department_id', '=', $managerDepartmentId)->get()->pluck('name', 'id');
