...
class LeaveController extends Controller
{
    public function index()
    {
        ...
        if (\Auth::user()->can('Manage Leave')) {
            ...
            if (\Auth::user()->type == 'employee') {
                ...
                return view('leave.index', compact('leaves','managerDepartmentId', 'managerBranchId'));
    ...
    public function create()
    {
        if (\Auth::user()->can('Create Leave')) {
            if (Auth::user()->type == 'employee') {
                ...
                $managerDepartmentId = User::where('id', '=', \Auth::user()->id)->get()->value('department_id');
                $managerBranchId = User::where('id', '=', \Auth::user()->id)->get()->value('branch_id');
                $employees = Employee::where('created_by', '=', \Auth::user()->creatorId())->where('branch_id', '=', $managerBranchId)->where('department_id', '=', $managerDepartmentId)->get()->pluck('name', 'id');
            }else{
    ...
    public function store(Request $request)
    {
        ...
        //*Remark Validation is not neccessary as the admin may or not may have concerns with the leave.
        //     // Add the 'remark' validation rule only if the user type is not 'employee'
        // $validator->sometimes('remark', 'required', function ($input) {
        ...
        //     });
            ...
            if (!empty($request->supporting_document)) {
                ...
                $filenameWithExt = $request->file('supporting_document')->getClientOriginalName();
                $filename        = pathinfo($filenameWithExt, PATHINFO_FILENAME);
                $extension       = $request->file('supporting_document')->getClientOriginalExtension();
                $fileNameToStore = $filename . '_' . time() . '.' . $extension;
                $dir = 'uploads/leaveDocuments/';
                $image_path = $dir . $fileNameToStore;
                if (\File::exists($image_path)) {
                    \File::delete($image_path);
                }
                $url = '';
                $path = \Utility::upload_file($request, 'supporting_document', $fileNameToStore, $dir, []);
                if ($path['flag'] == 1) {
                    $url = $path['url'];
                } else {
            ...
            if ($leave_type->days >= $total_leave_days) {
                ...
                $leave->supporting_document = !empty($request->supporting_document) ? $fileNameToStore : '';
        ...
        if (\Auth::user()->can('Edit Leave')) {
            if ($leave->created_by == \Auth::user()->creatorId()) {
                if (Auth::user()->type == 'employee') {
                    ...
                    $employees = Employee::where('employee_id', '=', $leave->employee_id)->get()->pluck('name', 'id');
                }
                ...
                //document upload - to show
                $leaves_toshow = LocalLeave::where('created_by', '=', \Auth::user()->creatorId())->get();
            if (\Auth::user()->type == 'employee') {
                $user     = \Auth::user();
                $employee = Employee::where('user_id', '=', $user->id)->first();
                $leaves_toshow   = LocalLeave::where('employee_id', '=', $employee->id)->get();
            } else {
                $leaves_toshow = LocalLeave::where('created_by', '=', \Auth::user()->creatorId())->get();
            }


                return view('leave.edit', compact('leave', 'employees', 'leavetypes', 'leaves_toshow'));
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
            ...
            if ($leave->created_by == Auth::user()->creatorId()) {
                $validator = \Validator::make(
                    ...
                    [
                        ...
                        'end_date' => 'required',
                        'leave_reason' => 'required',
                        // 'remark' => 'required', //*Remark Validation is not neccessary as the admin may or not may have concerns with the leave.
                    ]
                );
                if ($validator->fails()) {
                    $messages = $validator->getMessageBag();
                ...
                //supporting document edit by employee only
                if (\Auth::user()->type == 'employee') {
                if (!empty($request->supporting_document)) {
                    ...
                    $dir2 = storage_path('uploads/leaveDocuments/');

                //to delete the file from server.
                // Check if the document exists before attempting to delete
                if (!empty($leave->supporting_document)) {
                $file_path = $dir2 . $leave->supporting_document;
                ...
                if ($leave_type->days >= $total_leave_days) {
                    if (\Auth::user()->type == 'employee') {
                        ...
                        $leave->supporting_document = !empty($request->supporting_document) ? $fileNameToStore : $leave->supporting_document; //add this for employee edit.
                    } else {
                        $leave->employee_id      = $request->employee_id;
                    }
                    $leave->leave_type_id    = $request->leave_type_id;
    ...
    public function destroy(LocalLeave $leave)
    {
        if (\Auth::user()->can('Delete Leave')) {
            if ($leave->created_by == \Auth::user()->creatorId()) {
                $dir = storage_path('uploads/leaveDocuments/');

            //to delete the file from server.
                // Check if the document exists before attempting to delete
            if (!empty($leave->supporting_document)) {
                $file_path = $dir . $leave->supporting_document;
