    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('Manage TimeSheet'))
        {
            ...
            else
            {
                ...
                $employeesList = \Auth::user()->type=='manager'?Employee::where('created_by', \Auth::user()->creatorId())->where('branch_id',$managerBranchId)->where('department_id',$managerDepartmentId)->get()->pluck('name', 'user_id'):Employee::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'user_id');
                $employeesList->prepend('All', '');

            return view('timeSheet.index', compact('timeSheets', 'employeesList', 'managerDepartmentId', 'managerBranchId'));
